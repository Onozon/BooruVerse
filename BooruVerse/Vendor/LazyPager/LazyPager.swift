...
public struct Config<Element> {
    ...
    /// Optional width/height aspect ratio of the media inside each page.
    /// When set, zoom pan is clamped to the aspect-fit image rect so letterbox /
    /// pillarbox bars aren't treated as scrollable content.
    public var contentAspectRatio: ((Element) -> CGFloat?)?
