#!/usr/bin/env python3
"""
Графический интерфейс для Kemono.cr Parser v3
Создан на основе PyQt6 с расширенной системой кэширования
"""
...
class KemonoGUI(QMainWindow):
    """Главное окно приложения Kemono.cr Parser v3"""
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🎨 Kemono.cr Parser v3 - Advanced Caching")
    ...
    def setup_top_panel(self, parent_layout):
        ...
        # Чекбокс очистки кэша
        self.clear_cache_checkbox = QCheckBox("🗑️ Очистить кэш")
        ...
        top_layout.addWidget(self.clear_cache_checkbox)
    ...
    def load_posts_cache(self, artist_id):
        ...
        try:
            ...
            if cache_data.get('version') != 'v3':
                print(f"⚠️ Устаревший кэш для {artist_id}, будет пересоздан")
                return None
            ...
            print(f"📂 Кэш постов загружен: {artist_id} ({len(posts_data)} постов)")
            return posts_data, is_complete
        except Exception as e:
            print(f"❌ Ошибка загрузки кэша постов: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def save_media_cache(self, post_id, media_data):
        ...
        try:
            ...
            print(f"❌ Ошибка сохранения кэша медиа: {e}")
            return False
    ...
    def load_media_cache(self, post_id):
        ...
        try:
            ...
            # Проверяем версию кэша
            if cache_data.get('version') != 'v3':
                print(f"⚠️ Устаревший кэш медиа для {post_id}, будет пересоздан")
                return None

            media_data = cache_data.get('media', [])
            print(f"📂 Кэш медиа загружен: {post_id} ({len(media_data)} файлов)")
            return media_data
        except Exception as e:
            print(f"❌ Ошибка загрузки кэша медиа: {e}")
            return None
    ...
    def load_downloaded_files_cache(self):
        ...
        try:
            ...
            print(f"📂 Кэш скачанных файлов загружен: {len(self.downloaded_files_cache)} файлов")
            return self.downloaded_files_cache
        except Exception as e:
            print(f"❌ Ошибка загрузки кэша скачанных файлов: {e}")
            return set()
    ...
    def save_downloaded_files_cache(self):
        ...
        try:
            ...
            print(f"❌ Ошибка сохранения кэша скачанных файлов: {e}")
            return False
    ...
    def clear_artist_cache(self, artist_id):
        ...
        try:
            ...
            if posts_cache_path.exists():
                ...
                print(f"🗑️ Удален кэш постов: {artist_id}")
            ...
            print(f"✅ Кэш артиста {artist_id} очищен")
            return True

        except Exception as e:
            print(f"❌ Ошибка при очистке кэша артиста {artist_id}: {e}")
            return False
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    if temp_artist:
                        ...
                        self.status_updated.emit(f"🗑️ Очистка кэша для {artist_id}...")
                        if self.clear_artist_cache(artist_id):
                            self.status_updated.emit("✅ Кэш очищен, начинаем загрузку...")
                        else:
                            self.status_updated.emit("⚠️ Ошибка при очистке кэша")
                    else:
                        self.status_updated.emit("⚠️ Не удалось определить ID артиста для очистки кэша")
                except Exception as e:
                    print(f"❌ Ошибка при очистке кэша: {e}")
                    self.status_updated.emit("⚠️ Ошибка очистки кэша, продолжаем загрузку...")
            else:
                # Пользователь отменил очистку
                self.status_updated.emit("ℹ️ Очистка кэша отменена пользователем")
    ...
    def _analyze_posts_incremental(self, artist, existing_posts, last_cached_date):
        ...
        try:
            ...
            while True:
                ...
                try:
                    page_posts = self.parser.get_artist_posts_page(artist, page)
            ...
            try:
                ...
                if post_thumbnail:
                    ...
                    if cached_path:
                        ...
                        if not pixmap.isNull():
                            ...
                            self.update_thumbnail_text.emit(thumbnail_label, "🖼️ Нет превью")
                    else:
                    ...
                    if cached_path:
                        ...
                        if not pixmap.isNull():
                            ...
                            if "net::ERR_NAME_NOT_RESOLVED" in str(network_error) or "Connection" in str(network_error):
                                ...
                                self.update_thumbnail_text.emit(thumbnail_label, "🖼️ Ошибка загрузки")
                else:
                    self.update_thumbnail_text.emit(thumbnail_label, "🖼️ Нет превью")
                ...
                try:
                    if thumbnail_label is not None and hasattr(thumbnail_label, 'setText'):
                        self.update_thumbnail_text.emit(thumbnail_label, "🖼️ Ошибка")
    ...
    def on_media_load_error(self, post_id, error_message):
        ...
        try:
            ...
            self.media_error_label.setText(f"❌ {error_message}")
    ...
    def on_post_clicked(self, post, frame):
        ...
        post_title = self.get_post_attr(post, 'title', 'Без названия')
        self.media_label.setText(f"🖼️ Медиафайлы: {post_title}")
    ...
    def open_file_with_default_app(self, media_item):
        ...
        try:
            ...
            if not filepath.exists():
                print(f"📥 Файл не найден, скачиваем: {filepath.name}")
            ...
            print(f"📂 Открыт файл: {filepath.name}")
    ...
    def show_current_post_in_finder(self):
        ...
        try:
            ...
            print(f"📂 Открыта папка поста: {post_title}")
    ...
    def download_single_file_with_progress(self, media_item):
        ...
        try:
            ...
            print(f"✅ Файл скачан и открыт: {filepath.name}")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                if self.clear_all_cache():
                    ...
                    self.status_updated.emit("🗑️ Весь кэш очищен")
                else:
    ...
    def save_file_types_settings(self):
        ...
        try:
            ...
            print(f"❌ Ошибка сохранения настроек типов файлов: {e}")
            ...
            if settings_path.exists():
                with open(settings_path, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(loaded_settings, dict):
                        ...
                        print(f"📂 Настройки типов файлов загружены: {settings_path}")
                        print(f"🐛 DEBUG: Загруженные настройки: {self.file_types_settings}")
                    else:
                        print("⚠️ Неверный формат настроек типов файлов, используем значения по умолчанию")
            else:
                print("📂 Файл настроек типов файлов не найден, используем значения по умолчанию")
        except Exception as e:
            print(f"❌ Ошибка загрузки настроек типов файлов: {e}")
...
class FileTypesDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Группа для картинок
        images_group = QGroupBox("🖼️ Картинки")
