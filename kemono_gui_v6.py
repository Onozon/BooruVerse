...
from PyQt6.QtWidgets import (
    ...
    QListView, QTreeWidget, QTreeWidgetItem, QHeaderView, QGestureEvent, QPinchGesture
)
...
class MediaViewer(QMainWindow):
    """Окно для просмотра медиа файлов с поддержкой масштабирования"""
    ...
    def __init__(self, media_item, parent=None):
        ...
        self.min_scale_factor = 0.1  # Минимальный масштаб
        self.max_scale_factor = 5.0  # Максимальный масштаб
        self.gesture_start_scale = 1.0  # Масштаб на момент начала жеста
    ...
    def setup_ui(self):
        ...
        # QScrollArea для поддержки прокрутки при увеличении
        ...
        self.scroll_area.mousePressEvent = self.scrollAreaMousePressEvent
        self.scroll_area.mouseMoveEvent = self.scrollAreaMouseMoveEvent
        self.scroll_area.mouseReleaseEvent = self.scrollAreaMouseReleaseEvent
        self.scroll_area.setMouseTracking(True)  # Отслеживание движений мыши
        
        # Настраиваем обработку жестов
    ...
    def display_image(self, pixmap, save_original=False):
        """Отобразить изображение без рамок с текущим масштабом"""
        if pixmap.isNull():
            return
        ...
        # Получаем размеры scroll area (убираем рамки)
        scroll_size = self.scroll_area.size()
        available_width = scroll_size.width()
        available_height = scroll_size.height()

        if self.scale_factor == 1.0:
            # Автоматическое масштабирование для вмещения в окно
        scaled_pixmap = source_pixmap.scaled(
            ...
            # Ручное масштабирование с учетом scale_factor
            target_width = int(source_pixmap.width() * self.scale_factor)
            target_height = int(source_pixmap.height() * self.scale_factor)
        ...
        # Обновляем заголовок окна с информацией о масштабе
        filename = Path(self.media_item['filename']).name
        scale_percent = int(self.scale_factor * 100)
        self.setWindowTitle(f"Просмотр: {filename} ({scale_percent}%)")
    ...
    def gestureEvent(self, event):
        """Обработка pinch жестов"""
        try:
            # Получаем pinch жест
            pinch_gesture = event.gesture(Qt.GestureType.PinchGesture)
            
            if pinch_gesture and self.original_pixmap and not self.original_pixmap.isNull():
                # Преобразуем к QPinchGesture
                pinch = pinch_gesture
                
                # Получаем коэффициент масштабирования
                try:
                    scale_factor = pinch.totalScaleFactor()  # Общий масштаб с начала жеста
                except AttributeError:
                    scale_factor = pinch.scaleFactor()  # Текущий масштаб
                
                gesture_state = pinch.state()
                
                if gesture_state == Qt.GestureState.GestureStarted:
                    # Начало жеста
                    self.gesture_start_scale = self.scale_factor
                    print(f"[PINCH] Начало жеста, стартовый масштаб: {self.gesture_start_scale}")
                elif gesture_state == Qt.GestureState.GestureUpdated:
                    # Обновление жеста
                    new_scale = self.gesture_start_scale * scale_factor
                    
                    # Ограничиваем масштаб
                    if self.min_scale_factor <= new_scale <= self.max_scale_factor:
                        self.scale_factor = new_scale
                        self.display_image(self.original_pixmap)
                        print(f"[PINCH] Обновление: scale_factor={scale_factor:.2f}, new_scale={new_scale:.2f}")
                elif gesture_state == Qt.GestureState.GestureFinished:
                    print("[PINCH] Жест завершен")
                
                return True
                
        except Exception as e:
            print(f"[PINCH ERROR] Ошибка обработки жеста: {e}")
            
        return False
        
    def wheelEvent(self, event):
        """Обработка колесика мыши (более плавное)"""
        if self.original_pixmap and not self.original_pixmap.isNull():
            # Получаем направление прокрутки
            angle_delta = event.angleDelta().y()
            
            # Более плавное масштабирование
            scale_change = 1.05 if angle_delta > 0 else 1.0 / 1.05
            ...
            if self.min_scale_factor <= new_scale <= self.max_scale_factor:
                ...
                self.scale_factor = new_scale
                self.display_image(self.original_pixmap)
                print(f"[WHEEL] Масштаб: {old_scale:.2f} → {new_scale:.2f} (delta: {angle_delta})")
        else:
            super().wheelEvent(event)

    def keyPressEvent(self, event):
        """Обработка клавиш для дополнительного управления"""
        if self.original_pixmap and not self.original_pixmap.isNull():
            if event.key() == Qt.Key.Key_Plus or event.key() == Qt.Key.Key_Equal:
                # Увеличение масштаба
                new_scale = min(self.scale_factor * 1.25, self.max_scale_factor)
                if new_scale != self.scale_factor:
                    self.scale_factor = new_scale
                    self.display_image(self.original_pixmap)
            elif event.key() == Qt.Key.Key_Minus:
                # Уменьшение масштаба
                new_scale = max(self.scale_factor / 1.25, self.min_scale_factor)
                if new_scale != self.scale_factor:
                    self.scale_factor = new_scale
                    self.display_image(self.original_pixmap)
    ...
    def mousePressEvent(self, event):
        ...
        print(f"[DRAG] mousePressEvent: button={event.button()}, scale={self.scale_factor:.2f}")
        if event.button() == Qt.MouseButton.LeftButton and self.scale_factor > 1.0:
            ...
            print(f"[DRAG] Начало перетаскивания на позиции: {self.last_drag_pos}")
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.is_dragging and self.last_drag_pos:
            ...
            # Получаем scroll bars
            h_scroll = self.scroll_area.horizontalScrollBar()
            v_scroll = self.scroll_area.verticalScrollBar()
            
            # Обновляем позицию scroll bars
            old_h = h_scroll.value()
            old_v = v_scroll.value()
            h_scroll.setValue(old_h - delta.x())
            v_scroll.setValue(old_v - delta.y())
            
            print(f"[DRAG] Перетаскивание: delta=({delta.x()}, {delta.y()}), scroll: h {old_h}→{h_scroll.value()}, v {old_v}→{v_scroll.value()}")
            
            self.last_drag_pos = current_pos
        elif self.scale_factor > 1.0:
            # Показываем курсор открытой руки
    ...
    def mouseReleaseEvent(self, event):
        ...
        print(f"[DRAG] mouseReleaseEvent: button={event.button()}, was_dragging={self.is_dragging}")
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if self.scale_factor > 1.0:
                ...
                self.setCursor(Qt.CursorShape.ArrowCursor)
            print("[DRAG] Окончание перетаскивания")
        super().mouseReleaseEvent(event)
        
    def scrollAreaMousePressEvent(self, event):
        """Обработчик нажатия мыши в scroll area"""
        print(f"[DRAG] scrollAreaMousePressEvent: button={event.button()}, scale={self.scale_factor:.2f}")
        if event.button() == Qt.MouseButton.LeftButton and self.scale_factor > 1.0:
            self.is_dragging = True
            self.last_drag_pos = event.position().toPoint()
            self.setCursor(Qt.CursorShape.ClosedHandCursor)
            print(f"[DRAG] Начало перетаскивания в scroll area: {self.last_drag_pos}")
            event.accept()
        else:
            # Передаем событие дальше
            QScrollArea.mousePressEvent(self.scroll_area, event)

    def scrollAreaMouseMoveEvent(self, event):
        """Обработчик движения мыши в scroll area"""
        if self.is_dragging and self.last_drag_pos:
            # Вычисляем смещение
            current_pos = event.position().toPoint()
            delta = current_pos - self.last_drag_pos
            ...
            print(f"[DRAG] Перетаскивание в scroll area: delta=({delta.x()}, {delta.y()}), scroll: h {old_h}→{h_scroll.value()}, v {old_v}→{v_scroll.value()}")
    ...
    def scrollAreaMouseReleaseEvent(self, event):
        """Обработчик отпускания мыши в scroll area"""
        print(f"[DRAG] scrollAreaMouseReleaseEvent: button={event.button()}, was_dragging={self.is_dragging}")
        if event.button() == Qt.MouseButton.LeftButton:
            self.is_dragging = False
            self.last_drag_pos = None
            if self.scale_factor > 1.0:
                ...
                self.setCursor(Qt.CursorShape.ArrowCursor)
            print("[DRAG] Окончание перетаскивания в scroll area")
            event.accept()
        else:
            # Передаем событие дальше
    ...
    def resizeEvent(self, event):
        ...
        # Перемасштабируем изображение при изменении размера окна только если масштаб = 1.0
        if self.scale_factor == 1.0:
        if self.original_pixmap and not self.original_pixmap.isNull():
            self.display_image(self.original_pixmap)
...
class KemonoGUIv4(QMainWindow):
    ...
    artists_loaded_append = pyqtSignal(list, str)  # artists_list, page_type (для пагинации)
    posts_loaded_browser = pyqtSignal(list, str)  # posts_list, page_type
    posts_loaded_append = pyqtSignal(list, str)  # posts_list, page_type (для пагинации)
    ...
    def setup_browser_tab(self):
        ...
        # Основная область браузера теперь внутри под-вкладок

        self.tab_widget.addTab(browser_widget, "Браузер")

        # Инициализируем флаги ленивой загрузки
        self._artists_search_loaded = False
        self._artists_recent_loaded = False
        self._posts_search_loaded = False
        self._posts_popular_loaded = False
        self._posts_tags_loaded = False
    ...
    def setup_browser_sidebar(self, parent_layout):
        """Настройка горизонтальных под-вкладок браузера"""
        # Создаем основной QTabWidget для под-вкладок Авторы/Посты
        self.browser_sub_tabs = QTabWidget()
        self.browser_sub_tabs.setTabPosition(QTabWidget.TabPosition.North)  # Вкладки сверху
        self.browser_sub_tabs.setStyleSheet("""
            QTabWidget::pane {
        ...
        # Под-вкладка Авторы
        self.setup_artists_subtab()

        # Под-вкладка Посты
        self.setup_posts_subtab()

        # Подключаем сигнал изменения вкладки
        self.browser_sub_tabs.currentChanged.connect(self.on_subtab_changed)

        parent_layout.addWidget(self.browser_sub_tabs)

    def setup_artists_subtab(self):
        """Настройка под-вкладки Авторы с внутренними вкладками"""
        artists_widget = QWidget()
        artists_layout = QVBoxLayout(artists_widget)

        # Создаем внутренний QTabWidget для под-под-вкладок
        self.artists_inner_tabs = QTabWidget()
        self.artists_inner_tabs.setTabPosition(QTabWidget.TabPosition.North)
        self.artists_inner_tabs.setStyleSheet("""
            QTabWidget::pane {
                    border: none;
        ...
        # Под-под-вкладки для авторов
        self.setup_artists_search_tab()     # Поиск
        self.setup_artists_recent_tab()     # Недавние
        self.setup_artists_random_tab()     # Случайный

        # Подключаем сигнал изменения внутренней вкладки
        self.artists_inner_tabs.currentChanged.connect(self.on_artists_inner_tab_changed)

        artists_layout.addWidget(self.artists_inner_tabs)
        self.browser_sub_tabs.addTab(artists_widget, "👥 Авторы")

    def setup_posts_subtab(self):
        """Настройка под-вкладки Посты с внутренними вкладками"""
        posts_widget = QWidget()
        posts_layout = QVBoxLayout(posts_widget)

        # Создаем внутренний QTabWidget для под-под-вкладок
        self.posts_inner_tabs = QTabWidget()
        self.posts_inner_tabs.setTabPosition(QTabWidget.TabPosition.North)
        self.posts_inner_tabs.setStyleSheet("""
            QTabWidget::pane {
                    border: none;
                background-color: transparent;
            }
        ...
        # Под-под-вкладки для постов
        self.setup_posts_search_tab()      # Поиск
        self.setup_posts_popular_tab()     # Популярные
        self.setup_posts_tags_tab()        # Тэги
        self.setup_posts_random_tab()      # Случайный

        # Подключаем сигнал изменения внутренней вкладки
        self.posts_inner_tabs.currentChanged.connect(self.on_posts_inner_tab_changed)

        posts_layout.addWidget(self.posts_inner_tabs)
        self.browser_sub_tabs.addTab(posts_widget, "📝 Посты")

    def setup_artists_search_tab(self):
        """Настройка под-под-вкладки Поиск для авторов"""
        search_widget = QWidget()
        search_layout = QVBoxLayout(search_widget)

        # Панель поиска для авторов
        search_panel = QWidget()
        search_panel_layout = QHBoxLayout(search_panel)

        self.artists_search_input = QLineEdit()
        self.artists_search_input.setPlaceholderText("Поиск авторов...")
        search_panel_layout.addWidget(self.artists_search_input)

        # Выпадающий список сервисов
        self.artists_service_combo = QComboBox()
        self.artists_service_combo.addItems([
            "все сервисы",
            "patreon",
            "pixiv fanbox",
            "discord",
        ...
        self.artists_service_combo.setCurrentText("все сервисы")
        search_panel_layout.addWidget(self.artists_service_combo)

        self.artists_search_btn = QPushButton("Искать")
        self.artists_search_btn.clicked.connect(self.search_artists_from_input)
        search_panel_layout.addWidget(self.artists_search_btn)

        # Обработка Enter в поле поиска авторов
        self.artists_search_input.returnPressed.connect(self.search_artists_from_input)

        search_layout.addWidget(search_panel)

        # Создаем область результатов для этой вкладки
        self.artists_search_results_area = QScrollArea()
        self.artists_search_results_widget = QWidget()
        self.artists_search_results_layout = QVBoxLayout(self.artists_search_results_widget)

        self.artists_search_results_area.setWidget(self.artists_search_results_widget)
        self.artists_search_results_area.setWidgetResizable(True)
        search_layout.addWidget(self.artists_search_results_area)

        # Кнопка "Загрузить ещё"
        self.artists_search_load_more_btn = QPushButton("Загрузить ещё")
        self.artists_search_load_more_btn.clicked.connect(lambda: self.load_more_artists("search"))
        self.artists_search_load_more_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        search_layout.addWidget(self.artists_search_load_more_btn)
        
        # Инициализация offset для пагинации
        self.artists_search_offset = 0

        self.artists_inner_tabs.addTab(search_widget, "Поиск")

    def setup_artists_recent_tab(self):
        """Настройка под-под-вкладки Недавние для авторов"""
        recent_widget = QWidget()
        recent_layout = QVBoxLayout(recent_widget)

        # Создаем область результатов для этой вкладки
        self.artists_recent_results_area = QScrollArea()
        self.artists_recent_results_widget = QWidget()
        self.artists_recent_results_layout = QVBoxLayout(self.artists_recent_results_widget)

        self.artists_recent_results_area.setWidget(self.artists_recent_results_widget)
        self.artists_recent_results_area.setWidgetResizable(True)
        recent_layout.addWidget(self.artists_recent_results_area)

        # Кнопка "Загрузить ещё"
        self.artists_recent_load_more_btn = QPushButton("Загрузить ещё")
        self.artists_recent_load_more_btn.clicked.connect(lambda: self.load_more_artists("recent"))
        self.artists_recent_load_more_btn.setStyleSheet("""
            QPushButton {
                background-color: #007bff;
                color: white;
                border: none;
        ...
        """)
        recent_layout.addWidget(self.artists_recent_load_more_btn)
        
        # Инициализация offset для пагинации
        self.artists_recent_offset = 0

        self.artists_inner_tabs.addTab(recent_widget, "Недавние")

    def setup_artists_random_tab(self):
        """Настройка под-под-вкладки Случайный для авторов"""
        random_widget = QWidget()
        random_layout = QVBoxLayout(random_widget)

        # Создаем область результатов для этой вкладки
        self.artists_random_results_area = QScrollArea()
        self.artists_random_results_widget = QWidget()
        self.artists_random_results_layout = QVBoxLayout(self.artists_random_results_widget)

        self.artists_random_results_area.setWidget(self.artists_random_results_widget)
        self.artists_random_results_area.setWidgetResizable(True)
        random_layout.addWidget(self.artists_random_results_area)

        self.artists_inner_tabs.addTab(random_widget, "Случайный")

    def setup_posts_search_tab(self):
        """Настройка под-под-вкладки Поиск для постов"""
        search_widget = QWidget()
        search_layout = QVBoxLayout(search_widget)

        # Панель поиска для постов
        search_panel = QWidget()
        search_panel_layout = QHBoxLayout(search_panel)

        self.posts_search_input = QLineEdit()
        self.posts_search_input.setPlaceholderText("Поиск постов...")
        search_panel_layout.addWidget(self.posts_search_input)

        # Выпадающий список сервисов
        self.posts_service_combo = QComboBox()
        self.posts_service_combo.addItems([
            "все сервисы",
            "patreon",
            "pixiv fanbox",
            "discord",
        ...
        self.posts_service_combo.setCurrentText("все сервисы")
        search_panel_layout.addWidget(self.posts_service_combo)

        self.posts_search_btn = QPushButton("Искать")
        self.posts_search_btn.clicked.connect(self.search_posts_from_input)
        search_panel_layout.addWidget(self.posts_search_btn)

        # Обработка Enter в поле поиска постов
        self.posts_search_input.returnPressed.connect(self.search_posts_from_input)

        search_layout.addWidget(search_panel)

        # Создаем область результатов для этой вкладки
        self.posts_search_results_area = QScrollArea()
        self.posts_search_results_widget = QWidget()
        self.posts_search_results_layout = QVBoxLayout(self.posts_search_results_widget)

        self.posts_search_results_area.setWidget(self.posts_search_results_widget)
        self.posts_search_results_area.setWidgetResizable(True)
        search_layout.addWidget(self.posts_search_results_area)

        self.posts_inner_tabs.addTab(search_widget, "Поиск")

    def setup_posts_popular_tab(self):
        """Настройка под-под-вкладки Популярные для постов"""
        popular_widget = QWidget()
        popular_layout = QVBoxLayout(popular_widget)

        # Создаем область результатов для этой вкладки
        self.posts_popular_results_area = QScrollArea()
        self.posts_popular_results_widget = QWidget()
        self.posts_popular_results_layout = QVBoxLayout(self.posts_popular_results_widget)

        self.posts_popular_results_area.setWidget(self.posts_popular_results_widget)
        self.posts_popular_results_area.setWidgetResizable(True)
        popular_layout.addWidget(self.posts_popular_results_area)

        self.posts_inner_tabs.addTab(popular_widget, "Популярные")

    def setup_posts_tags_tab(self):
        """Настройка под-под-вкладки Тэги для постов"""
        tags_widget = QWidget()
        tags_layout = QVBoxLayout(tags_widget)

        # Создаем область результатов для этой вкладки
        self.posts_tags_results_area = QScrollArea()
        self.posts_tags_results_widget = QWidget()
        self.posts_tags_results_layout = QVBoxLayout(self.posts_tags_results_widget)

        self.posts_tags_results_area.setWidget(self.posts_tags_results_widget)
        self.posts_tags_results_area.setWidgetResizable(True)
        tags_layout.addWidget(self.posts_tags_results_area)

        self.posts_inner_tabs.addTab(tags_widget, "Тэги")

    def setup_posts_random_tab(self):
        """Настройка под-под-вкладки Случайный для постов"""
        random_widget = QWidget()
        random_layout = QVBoxLayout(random_widget)

        # Создаем область результатов для этой вкладки
        self.posts_random_results_area = QScrollArea()
        self.posts_random_results_widget = QWidget()
        self.posts_random_results_layout = QVBoxLayout(self.posts_random_results_widget)

        self.posts_random_results_area.setWidget(self.posts_random_results_widget)
        self.posts_random_results_area.setWidgetResizable(True)
        random_layout.addWidget(self.posts_random_results_area)

        self.posts_inner_tabs.addTab(random_widget, "Случайный")

    def on_subtab_changed(self, index):
        """Обработчик изменения основной под-вкладки (Авторы/Посты)"""
        print(f"[SUBTAB] Перешли на под-вкладку: {index}")

    def on_artists_inner_tab_changed(self, index):
        """Обработчик изменения внутренней вкладки авторов"""
        tab_names = ["Поиск", "Недавние", "Случайный"]
        tab_name = tab_names[index] if index < len(tab_names) else "Unknown"
        print(f"[ARTISTS_INNER] Перешли на вкладку: {tab_name}")

        # Выполняем загрузку данных при первом переходе
        if tab_name == "Поиск":
            if not hasattr(self, '_artists_search_loaded') or not self._artists_search_loaded:
                print("[ARTISTS_INNER] Первый переход на Поиск, загружаем данные")
                self.load_artists_page("main", 20)
                self._artists_search_loaded = True
            else:
                print("[ARTISTS_INNER] Повторный переход на Поиск, данные уже загружены")
        elif tab_name == "Недавние":
            if not hasattr(self, '_artists_recent_loaded') or not self._artists_recent_loaded:
                print("[ARTISTS_INNER] Первый переход на Недавние, загружаем данные")
                self.load_recent_artists()
                self._artists_recent_loaded = True
            else:
                print("[ARTISTS_INNER] Повторный переход на Недавние, данные уже загружены")
        elif tab_name == "Случайный":
            print("[ARTISTS_INNER] Переход на Случайный, выполняем поиск")
            self.find_random_user()

    def on_posts_inner_tab_changed(self, index):
        """Обработчик изменения внутренней вкладки постов"""
        tab_names = ["Поиск", "Популярные", "Тэги", "Случайный"]
        tab_name = tab_names[index] if index < len(tab_names) else "Unknown"
        print(f"[POSTS_INNER] Перешли на вкладку: {tab_name}")

        # Выполняем загрузку данных при первом переходе
        if tab_name == "Поиск":
            if not hasattr(self, '_posts_search_loaded') or not self._posts_search_loaded:
                print("[POSTS_INNER] Первый переход на Поиск, загружаем данные")
                self.load_posts_page("main", 20)
                self._posts_search_loaded = True
            else:
                print("[POSTS_INNER] Повторный переход на Поиск, данные уже загружены")
        elif tab_name == "Популярные":
            if not hasattr(self, '_posts_popular_loaded') or not self._posts_popular_loaded:
                print("[POSTS_INNER] Первый переход на Популярные, загружаем данные")
                self.load_popular_posts()
                self._posts_popular_loaded = True
            else:
                print("[POSTS_INNER] Повторный переход на Популярные, данные уже загружены")
        elif tab_name == "Тэги":
            if not hasattr(self, '_posts_tags_loaded') or not self._posts_tags_loaded:
                print("[POSTS_INNER] Первый переход на Тэги, загружаем данные")
                self.load_posts_by_tags()
                self._posts_tags_loaded = True
            else:
                print("[POSTS_INNER] Повторный переход на Тэги, данные уже загружены")
        elif tab_name == "Случайный":
            print("[POSTS_INNER] Переход на Случайный, выполняем поиск")
            self.load_posts_page("random", 1)

    def search_artists_from_input(self):
        """Поиск авторов из поля ввода на вкладке поиска авторов"""
        query = self.artists_search_input.text().strip()
        service_filter = self.artists_service_combo.currentText()

        if not query:
            print("[SEARCH] Пустой запрос поиска авторов")
            return

        print(f"[SEARCH] Поиск авторов: '{query}' с фильтром сервиса: {service_filter}")
        self.search_via_api(query, "artists", service_filter)

    def search_posts_from_input(self):
        """Поиск постов из поля ввода на вкладке поиска постов"""
        query = self.posts_search_input.text().strip()
        service_filter = self.posts_service_combo.currentText()

        if not query:
            print("[SEARCH] Пустой запрос поиска постов")
            return

        print(f"[SEARCH] Поиск постов: '{query}' с фильтром сервиса: {service_filter}")
        self.search_via_api(query, "posts", service_filter)
    ...
    def set_search_type(self, search_type, button_name, url):
        ...
        if hasattr(self, 'browser_search_input'):
            placeholder_text = f"Поиск {search_type}..."
            self.browser_search_input.setPlaceholderText(placeholder_text)
            print(f"[UI] Обновлен placeholder: {placeholder_text}")

        # Автоматически выполняем действия в зависимости от кнопки
        if search_type == "artists":
            if button_name == "Поиск":
                print("[NAV] Загружаем подборку популярных авторов")
                self.load_artists_page("main", 20)  # Загружаем 20 популярных авторов
            elif button_name == "Недавние":
                print("[NAV] Загружаем недавно обновленных авторов")
                self.load_recent_artists()
            elif button_name == "Случайный":
                print("[NAV] Выбираем случайного автора")
                self.find_random_user()
        elif search_type == "posts":
            if button_name == "Поиск":
                print("[NAV] Загружаем подборку постов")
                # TODO: реализовать загрузку популярных постов
                self.status_updated.emit("Поиск постов пока не реализован")
            elif button_name == "Недавние":
                print("[NAV] Загружаем недавние посты")
                self.status_updated.emit("Недавние посты пока не реализованы")
            elif button_name == "Случайный":
                print("[NAV] Выбираем случайный пост")
                self.status_updated.emit("Случайные посты пока не реализованы")
    ...
    def setup_browser_main_area(self, parent_layout):
        """Настройка основной области браузера с результатами"""
        main_area = QWidget()
    ...
    def load_artists_page_random_viewer(self, limit=1):
        ...
        def load_worker():
            temp_parser = None
            try:
                # Используем главный парсер, если есть, или создаем временный
                if hasattr(self, 'parser') and self.parser is not None:
                    parser_to_use = self.parser
                else:
                    temp_parser = KemonoParser(use_selenium=True, headless=True)
                    parser_to_use = temp_parser

                # Получаем случайных авторов
                artists = parser_to_use.get_artists_page("https://kemono.cr/artists/random", limit=limit)
                if artists:
    ...
    def load_posts_page_random_viewer(self, limit=1):
        ...
        def load_worker():
            ...
            try:
                ...
                posts = parser_to_use.get_posts_page("https://kemono.cr/posts/random", limit=limit)
                if posts:
                    first_post = posts[0]
    ...
    def load_artists_page(self, page_type, limit=50, offset=0, append=False):
        """Загрузка страницы авторов"""
        print(f"[ARTISTS] Загрузка страницы типа: {page_type}, лимит: {limit}, offset: {offset}, append: {append}")

        try:
            # Показываем индикатор загрузки
            if append:
                self.status_updated.emit("Загрузка дополнительных авторов...")
            else:
                self.status_updated.emit("Загрузка списка авторов...")

            # Загружаем данные в фоновом потоке
            self.load_artists_async(page_type, limit, offset, append)

        except Exception as e:
            print(f"[ARTISTS] Ошибка загрузки страницы: {e}")
            self.status_updated.emit(f"Ошибка загрузки: {str(e)}")

    def load_artists_async(self, page_type, limit, offset=0, append=False):
        ...
        def load_worker():
            try:
                ...
                if page_type in page_urls:
                    page_url = page_urls[page_type]
                    print(f"[ARTISTS] Загружаем страницу: {page_url} (лимит: {limit}, offset: {offset})...")
                    # Для пагинации через Selenium нужно добавить параметр offset в URL или скроллить
                    # Пока используем limit * (offset/50 + 1) для получения нужного количества
                    total_limit = limit + offset
                    all_artists = self.parser.get_artists_page(page_url, limit=total_limit)
                    # Берем только нужный срез
                    artists = all_artists[offset:offset+limit] if offset < len(all_artists) else []
                else:
                ...
                if not artists:
                    ...
                    self.status_updated.emit("Больше авторов нет")
                    return
                ...
                # Отправляем результаты в главный поток
                if append:
                    self.artists_loaded_append.emit(artists, page_type)
                else:
    ...
    def browser_search(self):
        """Поиск в браузере с учетом текущего типа и сервиса"""
        query = self.browser_search_input.text().strip()
        selected_service = self.service_combo.currentText()

        if query:
            print(f"[SEARCH] Начинаем API поиск '{query}' в разделе {self.current_search_type}, сервис: {selected_service}")
            self.search_via_api(query, self.current_search_type, selected_service)
        else:
            # Если поле пустое, загружаем подборку
            print(f"[SEARCH] Поле поиска пустое, загружаем подборку для {self.current_search_type}")
    ...
    def search_via_api(self, query, search_type, service_filter="все сервисы"):
        """Поиск через API с сессионной кукой"""
        print(f"[API] Начинаем поиск '{query}' типа {search_type}, сервис: {service_filter}")

        # Показываем статус загрузки
        self.status_updated.emit(f"Поиск {search_type}: {query}...")
            ...
            try:
                ...
                try:
                    with open('session_cookie.txt', 'r', encoding='utf-8') as f:
                        ...
                        print(f"[API] Сессионная кука загружена: {session_cookie[:20]}...")
                except Exception as e:
                    print(f"[API] Ошибка чтения сессионной куки: {e}")
                    session_cookie = None

                # Определяем URL API в зависимости от типа поиска
                if search_type == "artists":
                    api_url = f"https://kemono.cr/api/v1/creators?q={query}"
                elif search_type == "posts":
                    # Для постов используем поиск по авторам (пока)
                    api_url = f"https://kemono.cr/api/v1/creators?q={query}"
                else:
                    self.status_updated.emit(f"Неизвестный тип поиска: {search_type}")
                    return

                print(f"[API] Запрос к: {api_url}")

                # Делаем запрос с правильными заголовками и кукой
                headers = {
                    'Accept': 'text/css',
                ...
                print(f"[API] HTTP статус: {response.status_code}")

                if response.status_code == 200:
                    try:
                        # API возвращает JSON с Content-Type: text/css для обхода ботов
                        data = response.json()
                        print(f"[API] Получено {len(data)} результатов")

                        if search_type == "artists":
                            # Фильтруем результаты по запросу и сервису
                            filtered_data = []
                            ...
                            for item in data:
                                ...
                                query_match = (query_lower in name or
                                             query_lower in service or
                                             query_lower in item_id)

                                # Проверяем фильтр сервиса
                                service_match = True
                                if service_filter != "все сервисы":
                                    # Маппинг названий из combo box к API названиям
                                    service_mapping = {
                                        "pixiv fanbox": "fanbox",
                                        "subscribe star": "subscribestar"
                                    }
                                    api_service = service_mapping.get(service_filter, service_filter)
                                    service_match = (service == api_service)

                                if query_match and service_match:
                                    filtered_data.append(item)

                            print(f"[API] Найдено {len(filtered_data)} совпадений из {len(data)} авторов (сервис: {service_filter})")

                            # Преобразуем данные API в формат Artist
                            artists = []
                            for item in filtered_data[:20]:  # Ограничиваем до 20 результатов
                                try:
                                    print(f"[API] Создаем Artist из данных: id={item.get('id')}, name={item.get('name')}, service={item.get('service')}")
                                    artist = Artist(
                                        id=str(item.get('id', '')),
                                        service=item.get('service', ''),
                                        name=item.get('name', ''),
                                        indexed=str(item.get('indexed', 0)),
                                        updated=str(item.get('updated', 0)),
                                        url=f"https://kemono.cr/{item.get('service', '')}/user/{item.get('id', '')}"
                                    )
                                    print(f"[API] Artist создан: {artist.name} ({artist.service})")
                                    artists.append(artist)
                                except Exception as e:
                                    print(f"[API] Ошибка создания Artist: {e}")
                                    import traceback
                                    traceback.print_exc()
                                    continue

                            print(f"[API] Успешно обработано {len(artists)} авторов")
                            self.artists_loaded.emit(artists, "search")

                        else:
                            self.status_updated.emit(f"Поиск {search_type} пока не реализован")

                    except Exception as e:
                        print(f"[API] Ошибка парсинга JSON: {e}")
                        self.status_updated.emit(f"Ошибка обработки результатов: {str(e)}")
                else:
                    print(f"[API] Ошибка HTTP: {response.status_code}")
                    if response.status_code == 403:
                        self.status_updated.emit("Доступ запрещен. Проверьте сессионную куку.")
                    else:
                        self.status_updated.emit(f"Ошибка сервера: {response.status_code}")

            except Exception as e:
                print(f"[API] Ошибка поиска: {e}")
    ...
    def find_random_user(self):
        """Найти случайного автора через API"""
        print("[RANDOM] Начинаем поиск случайного автора")

        # Показываем статус загрузки
        self.status_updated.emit("Поиск случайного автора...")
        ...
        def random_worker():
            try:
                ...
                try:
                    with open('session_cookie.txt', 'r', encoding='utf-8') as f:
                        session_cookie = f.read().strip()
                        print(f"[RANDOM] Сессионная кука загружена")
                except Exception as e:
                    print(f"[RANDOM] Ошибка чтения сессионной куки: {e}")
                    session_cookie = None

                # Получаем все creators через API
                api_url = "https://kemono.cr/api/v1/creators"
                print(f"[RANDOM] Запрос к: {api_url}")
                ...
                response = requests.get(api_url, headers=headers, cookies=cookies, timeout=15)
                print(f"[RANDOM] HTTP статус: {response.status_code}")

                if response.status_code == 200:
                    try:
                        data = response.json()
                        print(f"[RANDOM] Получено {len(data)} авторов")

                        if data:
                            # Выбираем случайного автора
                            random_artist = random.choice(data)
                            print(f"[RANDOM] Выбран случайный автор: {random_artist.get('name', 'Unknown')}")

                            # Преобразуем в формат Artist
                            artist = Artist(
                                id=random_artist.get('id', ''),
                                service=random_artist.get('service', ''),
                                name=random_artist.get('name', ''),
                                indexed=random_artist.get('indexed', 0),
                                updated=random_artist.get('updated', 0),
                                url=f"https://kemono.cr/{random_artist.get('service', '')}/user/{random_artist.get('id', '')}"
                            )

                            print(f"[RANDOM] Открываем автора: {artist.name} ({artist.service})")
                            self.artists_loaded.emit([artist], "random")

                        else:
                            self.status_updated.emit("Не найдено авторов")

                    except Exception as e:
                        print(f"[RANDOM] Ошибка парсинга JSON: {e}")
                        self.status_updated.emit(f"Ошибка обработки: {str(e)}")
                else:
                    print(f"[RANDOM] Ошибка HTTP: {response.status_code}")
                    self.status_updated.emit(f"Ошибка сервера: {response.status_code}")

            except Exception as e:
                print(f"[RANDOM] Ошибка поиска случайного автора: {e}")
    ...
    def load_more_artists(self, page_type):
        """Загрузить больше авторов с пагинацией"""
        print(f"[PAGINATION] Загрузка больше авторов для типа: {page_type}")
        
        if page_type == "search" or page_type == "main":
            self.artists_search_offset += 50
            print(f"[PAGINATION] Новый offset для search: {self.artists_search_offset}")
            self.load_artists_page("main", limit=50, offset=self.artists_search_offset, append=True)
        elif page_type == "recent":
            self.artists_recent_offset += 50
            print(f"[PAGINATION] Новый offset для recent: {self.artists_recent_offset}")
            self.load_recent_artists(offset=self.artists_recent_offset, append=True)

    def load_recent_artists(self, offset=0, append=False):
        """Загрузить недавно обновленных авторов через API"""
        print("[RECENT] Начинаем загрузку недавно обновленных авторов")

        # Показываем статус загрузки
        self.status_updated.emit("Загрузка недавних авторов...")
        ...
        def recent_worker():
            try:
                ...
                try:
                    with open('session_cookie.txt', 'r', encoding='utf-8') as f:
                        session_cookie = f.read().strip()
                        print(f"[RECENT] Сессионная кука загружена")
                except Exception as e:
                    print(f"[RECENT] Ошибка чтения сессионной куки: {e}")
                    session_cookie = None

                # Получаем все creators через API
                api_url = "https://kemono.cr/api/v1/creators"
                print(f"[RECENT] Запрос к: {api_url}")
                ...
                response = requests.get(api_url, headers=headers, cookies=cookies, timeout=15)
                print(f"[RECENT] HTTP статус: {response.status_code}")

                if response.status_code == 200:
                    try:
                        data = response.json()
                        print(f"[RECENT] Получено {len(data)} авторов")

                        if data:
                            # Сортируем по дате обновления (updated) - самые свежие первыми
                            sorted_data = sorted(data, key=lambda x: x.get('updated', 0), reverse=True)
                            # Применяем offset и берем 50 элементов
                            recent_artists = sorted_data[offset:offset+50]

                            print(f"[RECENT] Выбрано {len(recent_artists)} недавно обновленных авторов (offset={offset})")

                            # Преобразуем в формат Artist
                            artists = []
                            for item in recent_artists:
                                try:
                                    artist = Artist(
                                        id=str(item.get('id', '')),
                                        service=item.get('service', ''),
                                    ...
                                    print(f"[RECENT] Ошибка создания Artist: {e}")
                                    continue

                            print(f"[RECENT] Успешно обработано {len(artists)} авторов")
                            # Отправляем с флагом append
                            if append:
                                self.artists_loaded_append.emit(artists, "recent")
                            else:
                                self.artists_loaded.emit(artists, "recent")

                        else:
                            self.status_updated.emit("Не найдено авторов")

                    except Exception as e:
                        print(f"[RECENT] Ошибка парсинга JSON: {e}")
                        self.status_updated.emit(f"Ошибка обработки: {str(e)}")
                else:
                    print(f"[RECENT] Ошибка HTTP: {response.status_code}")
                    self.status_updated.emit(f"Ошибка сервера: {response.status_code}")

            except Exception as e:
                print(f"[RECENT] Ошибка загрузки недавних авторов: {e}")
                self.status_updated.emit(f"Ошибка: {str(e)}")

        # Запускаем в отдельном потоке
        import threading
        thread = threading.Thread(target=recent_worker, daemon=True)
        thread.start()

    def load_popular_posts(self):
        """Загрузить популярные посты"""
        print("[POPULAR] Начинаем загрузку популярных постов")
        self.load_posts_page("popular", 20)

    def load_posts_by_tags(self):
        """Загрузить посты по тэгам (пока просто популярные)"""
        print("[TAGS] Начинаем загрузку постов по тэгам")
        # Пока используем популярные посты, так как API для тэгов может быть другим
        self.load_posts_page("tags", 20)

    def find_random_post(self):
        """Найти случайный пост (уже реализовано через load_posts_page)"""
        print("[RANDOM_POST] Поиск случайного поста")
        self.load_posts_page("random", 1)
    ...
    def load_post_from_url(self):
        ...
        print(f"[POST] load_post_from_url вызвана, URL из поля: '{url}'")
        if url:
            print(f"[POST] Загрузка поста: {url}")
            # Переключаемся на вкладку просмотра (если еще не там)
    ...
    def create_artist_card(self, artist_data):
        ...
        # Загружаем аватарку в фоновом потоке
        avatar_url = getattr(artist_data, 'avatar', '')
        if not avatar_url:
            avatar_url = f"https://img.kemono.cr/icons/{service}/{artist_id}"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not pixmap.isNull():
                        ...
                        self.set_avatar_placeholder(avatar_label, name)
                else:
                    self.set_avatar_placeholder(avatar_label, name)
            except:
                self.set_avatar_placeholder(avatar_label, name)
        ...
        id_label = QLabel(f"ID: {artist_id}")
    ...
    def open_post_in_viewer(self, post_data):
        ...
        if url:
            print(f"[POST] open_post_in_viewer: найден URL: {url}")
            # Сначала переключаемся на вкладку просмотра
            self.tab_widget.setCurrentIndex(1)
            # Затем устанавливаем URL в поле
            self.post_url_input.setText(url)
            print(f"[POST] open_post_in_viewer: URL установлен в поле: {self.post_url_input.text()}")
            # И только потом загружаем
            self.load_post_from_url()
            print(f"[POST] Открытие поста в просмотре: {url}")
        else:
            print(f"[POST] open_post_in_viewer: URL не найден в post_data")
    ...
    def setup_connections(self):
        ...
        self.artists_loaded_append.connect(self.on_artists_loaded_append)
        self.posts_loaded_browser.connect(self.on_posts_loaded_browser)
        self.posts_loaded_append.connect(self.on_posts_loaded_append)
    ...
    def download_and_cache_preview(self, url, size=(100, 80), preview_type="media", max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Кэшируем
                result = self.cache_preview(url, pil_image, preview_type)
                if result:
                    print(f"✅ Превью загружено и закешировано: {url}")
                return result
    ...
    def on_posts_loaded(self, posts):
        ...
        print(f"📨 on_posts_loaded вызван с {len(posts) if posts else 0} постами")
        ...
        self.all_posts = posts  # Устанавливаем self.all_posts
        print(f"💾 self.all_posts установлен: {len(posts) if posts else 0} постов")
    ...
    def on_media_loaded(self, media):
        ...
        print(f"🎥 on_media_loaded вызван с {len(media) if media else 0} медиа файлами")
    ...
    def on_artists_loaded(self, artists, page_type):
        ...
        # Отображаем результаты (очистка происходит внутри display_artists_results)
        self.display_artists_results(artists, page_type, append=False)

        # Обновляем статус
        self.status_updated.emit(f"Загружено {len(artists)} авторов")

    def on_artists_loaded_append(self, artists, page_type):
        """Обработчик загрузки списка авторов с добавлением (пагинация)"""
        print(f"[ARTISTS] Добавление {len(artists)} авторов типа: {page_type}")

        # Добавляем результаты к существующим
        self.display_artists_results(artists, page_type, append=True)

        # Обновляем статус
        self.status_updated.emit(f"Добавлено {len(artists)} авторов")
    ...
    def on_posts_loaded_browser(self, posts, page_type):
        ...
        # Отображаем результаты (очистка происходит внутри display_posts_results)
        self.display_posts_results(posts, page_type)

        # Обновляем статус
        self.status_updated.emit(f"Загружено {len(posts)} постов")

    def on_posts_loaded_append(self, posts, page_type):
        """Обработчик загрузки списка постов с добавлением (пагинация)"""
        print(f"[POSTS] Добавление {len(posts)} постов типа: {page_type}")
        # TODO: реализовать display_posts_results с append=True
        self.status_updated.emit(f"Добавлено {len(posts)} постов")

    def display_posts_results(self, posts, page_type):
        """Отображение результатов поиска постов в соответствующей под-вкладке"""
        print(f"[POSTS] Отображение {len(posts)} постов типа: {page_type}")

        # Определяем целевую область результатов в зависимости от типа
        if page_type == "main":
            target_layout = self.posts_search_results_layout
        elif page_type == "popular":
            target_layout = self.posts_popular_results_layout
        elif page_type == "tags":
            target_layout = self.posts_tags_results_layout
        elif page_type == "random":
            target_layout = self.posts_random_results_layout
        else:
            print(f"[POSTS] Неизвестный тип страницы: {page_type}")
            return
    ...
    def clear_layout(self, layout):
        """Очистка всех виджетов из layout'а"""
        if layout is None:
            return
    ...
    def display_artists_results(self, artists, page_type, append=False):
        """Отображение результатов поиска авторов в соответствующей под-вкладке"""
        print(f"[ARTISTS] Отображение {len(artists)} авторов типа: {page_type}, append={append}")

        # Определяем целевую область результатов в зависимости от типа
        if page_type == "main" or page_type == "search":
            target_layout = self.artists_search_results_layout
        elif page_type == "recent":
            target_layout = self.artists_recent_results_layout
        elif page_type == "random":
            target_layout = self.artists_random_results_layout
        else:
            print(f"[ARTISTS] Неизвестный тип страницы: {page_type}")
            return

        # Очищаем предыдущие результаты только если не добавляем
        if not append:
            self.clear_layout(target_layout)

        # Удаляем старую растяжку, если она есть
        if target_layout.count() > 0:
            last_item = target_layout.itemAt(target_layout.count() - 1)
            if last_item and last_item.spacerItem():
                target_layout.removeItem(last_item)
    ...
    def set_avatar_placeholder(self, label, name):
        """Устанавливает заглушку для аватарки с инициалами"""
        try:
            # Создаем изображение с инициалами
            from PyQt6.QtGui import QPixmap, QPainter, QColor, QFont
            from PyQt6.QtCore import Qt
            
            # Определяем размер на основе размера label
            size = label.width() if label.width() > 0 else 120
            ...
            # Используем крупный шрифт для инициалов (пропорциональный размеру)
            font = QFont()
            font.setPointSize(int(size * 0.35))  # 35% от размера
            font.setBold(True)
    ...
    def create_artist_card_browser(self, artist):
        ...
        # Загружаем аватарку в фоновом потоке
        avatar_url = getattr(artist, 'avatar', '')
        if not avatar_url:
            avatar_url = f"https://img.kemono.cr/icons/{artist.service}/{artist.id}"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not pixmap.isNull():
                        ...
                        # Если аватарка не загрузилась, показываем заглушку с инициалами
                        self.set_avatar_placeholder(avatar_label, artist.name)
                else:
                    self.set_avatar_placeholder(avatar_label, artist.name)
            except:
                self.set_avatar_placeholder(avatar_label, artist.name)
        ...
        # Отладка
        print(f"[CARD] Создание карточки для: name={artist.name}, service={artist.service}, id={artist.id}")
    ...
    def update_posts_display(self):
        ...
        print(f"🔄 update_posts_display: {len(self.all_posts) if self.all_posts else 0} постов")
        ...
        if not self.all_posts:
            print("❌ Нет постов для отображения")
            return
        ...
        for i, post in enumerate(page_posts):
            print(f"📋 Создание карточки {i+1}/{len(page_posts)}: {self.get_post_attr(post, 'id', 'unknown')}")
    ...
    def create_post_card(self, post, grid_layout, row, col):
        ...
        if post_thumbnail:
            # Временно добавим лог для проверки
            print(f"🖼️ Начинаем загрузку превью для поста {self.get_post_attr(post, 'id', 'unknown')}")
    ...
    def update_media_display(self):
        ...
        print(f"🎨 update_media_display: {len(self.current_media) if self.current_media else 0} медиа файлов")
        ...
        if not self.current_media:
            print("❌ Нет медиа для отображения")
            return
        ...
        for i, media_item in enumerate(self.current_media):
            print(f"🖼️ Создание карточки медиа {i+1}/{len(self.current_media)}: {media_item.get('filename', 'unknown')}")
    ...
    def load_media_thumbnail_async(self, media_item, thumbnail_label):
        ...
        print(f"🎬 Начинаем загрузку превью медиа: {media_item.get('filename', 'unknown')}")

        def load_thumbnail():
            try:
                print(f"🔄 Загрузка превью медиа в потоке: {media_item.get('filename', 'unknown')}")
                # Проверяем, что thumbnail_label все еще существует
                if thumbnail_label is None or not hasattr(thumbnail_label, 'setPixmap'):
                    print("❌ thumbnail_label медиа не существует")
                    return
...
class FileTypesDialog(QDialog):
    ...
    def closeEvent(self, event):
        ...
        print("[CLEANUP] Закрытие приложения...")
        
        # Закрываем парсер, если он существует
        if hasattr(self, 'parser') and self.parser is not None:
            try:
                print("[CLEANUP] Закрываем парсер...")
                self.parser.close()
                print("[CLEANUP] Парсер закрыт успешно")
            except Exception as e:
                print(f"[CLEANUP] Ошибка при закрытии парсера: {e}")
